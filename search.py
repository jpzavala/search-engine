...
def common(posting_lists): # contender/webpage that is likely to be at the top results 
    temp = []
    for lst in posting_lists:
        s = {i['id'] for i in lst} # next(iter(dict())) is a fast trick to get the key in a dict with just one key such as our postings
        if len(s):
            temp.append(s)
...
# getter uses binary search to find the value mapped to the specified docID in a list of postings fast
# ex getter(1, [{1:2}, {3:7}]) returns 2
...
def process_stopwords(tokens): # stop word processing
    if len(tokens) <= 1: # return if query is one word
        return tokens
    else:
        processed = [t for t in tokens if t not in {'for', 'by', 'are', 'of', 'the', 'is', 'at', 'and', 'in', 'on', 'to'}] # design decision 
        if len(processed) == 0: # query is only stop words
            return [tokens[0]] 
        else: # query has at least one good word
            return processed
                
def tfidf_ranking(terms, termsnfreqs, posting_lists): # ranking
    ranking = PriorityQueue() # heap
    idfs = dict() # holds idfs
    query_vector = list()

    if len(posting_lists) == 1: # if the query is just one word, idf is not gonna be used #tf
        ...
        for i in range(len(terms)):
            try:
                idfs[terms[i]] = math.log(float(53792)/len(posting_lists[i])) # calculating idf value
        ...
        query_vector = [i/query_length for i in query_vector] # normalizing the query vector, this is for cosine similarity

    #For each document, create document vector then compute score
    for n in in_common: 
        doc_vector = list()
        score = 0
        for i in range(len(terms)): # for every word in the query 
            ...
            if pos['tf'] > 0:
                ...
                doc_vector.append(0) # add 0 
        if sum(doc_vector) > 0:
            doc_length = math.sqrt(sum([i**2 for i in doc_vector]))
            doc_vector = [i/doc_length for i in doc_vector] # normalizing vector
            score += sum([query_vector[i] * doc_vector[i] for i in range(len(query_vector))]) # cosine similarity
        else:
            score = 0
        ranking.put((-1 * score, n))
    
    #Get the docIDs of the highest score values found
